...
QuizStudent.findById = (student_fkid, quiz_fkid, result) => {
    sql.query(`SELECT * FROM students_quiz WHERE student_fkid = ${student_fkid} AND quiz_fkid = ${quiz_fkid}`, (err, res) => {
      ...
      if (res.length) {
        console.log("found QuizStudent: ", res);
        result(null, res);
        return;
      }
  ...
  QuizStudent.findById1 = (student_fkid, result) => {
    sql.query(`SELECT * FROM students_quiz INNER JOIN students ON students_quiz.student_fkid = student_id INNER JOIN students_answers ON students_answers.student_fkid = student_id INNER JOIN answers ON answers.id = students_answers.answer_fkid WHERE correct = "pending" AND answer_text_fk != "" AND students_quiz.student_fkid = ${student_fkid}`, (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(err, null);
        return;
